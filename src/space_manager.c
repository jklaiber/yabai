...
{
    if (cur_sid == sid)
        return SPACE_OP_ERROR_SAME_SPACE;
    bool is_in_mc = g_mission_control_active;
    if (is_in_mc)
        return SPACE_OP_ERROR_IN_MISSION_CONTROL;
    ...
    if (is_animating)
        return SPACE_OP_ERROR_DISPLAY_IS_ANIMATING;
...
{
    if (sm->did_begin)
        return space_manager_find_view(sm, sid);
...
uint64_t space_manager_active_space(void)
{
    ...
    if (window)
        did = window_display_id(window);
    if (!did)
        did = display_manager_active_display_id();
    if (!did)
        return 0;
...
enum space_op_error space_manager_swap_space_with_space(uint64_t acting_sid, uint64_t selector_sid)
{
    ...
    if (acting_did != selector_did)
        return SPACE_OP_ERROR_SAME_DISPLAY;
...
enum space_op_error space_manager_move_space_to_display(struct space_manager *sm, uint64_t sid, uint32_t did)
{
    ...
    if (is_in_mc)
        return SPACE_OP_ERROR_IN_MISSION_CONTROL;
    if (!sid)
        return SPACE_OP_ERROR_MISSING_SRC;
